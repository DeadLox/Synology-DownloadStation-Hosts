...
class SynoFileHostingWipfiles {
	private $Url;
	private $Username;
	private $Password;
	private $HostInfo;
	private $WF_COOKIE_JAR = '/tmp/wipfiles.cookie';
	private $LOGIN_URL = "http://wipfiles.net/?op=login";
	...
	private function performLogin() {
		$ret = LOGIN_FAIL;
		//Save cookie file
		//op=login&redirect=http%3A%2F%2Fwipfiles.net%2F&login=&password=
		$PostData = array('op'=>'login',
						'redirect'=>'http%3A%2F%2Fwipfiles.net%2F',
						'login'=>$this->Username,
		...
		curl_setopt($curl, CURLOPT_COOKIEJAR, $this->WF_COOKIE_JAR);
		...
		//xfss is wipfiles logged in cookie value
		if (FALSE != $LoginInfo && file_exists($this->WF_COOKIE_JAR)) {
			$cookieData = file_get_contents ($this->WF_COOKIE_JAR);
	...
	private function getPremiumDownloadLink() {
		...
		curl_setopt($curl, CURLOPT_COOKIEFILE, $this->WF_COOKIE_JAR);
		curl_setopt($curl, CURLOPT_RETURNTRANSFER, true); 
